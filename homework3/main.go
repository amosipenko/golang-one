...
func calcArgumentsFromString(str string) (float64, string, float64) {
	...
	for i := 0; i < len(str); i++ {
		...
		if str[i] >= 48 && str[i] <= 57 || str[i] == 46 {
			...
			// отдельно обрабатываем минус, т.к. он может относиться к аргументу иди быть операцией
			if aStr == "" && oper == "" {
		...
		} else if str[i] == 43 || str[i] == 45 || str[i] == 42 || str[i] == 47 || str[i] == 37 || str[i] == 33 || str[i] == 94 || str[i] == 35 || str[i] == 60 || str[i] == 62 {
